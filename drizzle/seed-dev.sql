-- Seed data: demo article for LOCAL DEV / STAGING ONLY.
--
-- DO NOT run on production D1 — `published_at` is set, so the article
-- becomes immediately visible on the homepage and gets pushed to IndexNow
-- if the user re-publishes it. That's noise for a real fork.
--
-- Run after seed.sql:
--   bun run db:seed:dev:local      (local D1)
--   bun run db:seed:dev:staging    (staging D1 only — never prod)
--
-- Idempotent: INSERT OR IGNORE on a fixed ULID, so re-running is safe.
